let CONFIG = {
  ...
  // BTHome door/window sensor(s) that will report to this device
  sensorMACs: [
    "7c:c6:b6:77:5d:00"                 // Shelly BLU Door/Window sensor
  ],
  switchId: 0                           // relay channel
};

let lastPacketIds = {};                 // debounce per sensor using pid
let currentSwitchOn = null;             // track to avoid redundant calls
...
// drive the relay to match the door state
function applyDoorState(isOpen) {
  if (currentSwitchOn === isOpen) return;

  Shelly.call("Switch.Set", { id: CONFIG.switchId, on: isOpen }, function(res, err) {
    if (err) {
      print("Switch.Set error: " + JSON.stringify(err));
    } else {
      currentSwitchOn = isOpen;
      print("Door " + (isOpen ? "OPEN" : "CLOSED") + " -> Switch " + (isOpen ? "ON" : "OFF"));
    }
  });
}
...
function init() {
  ...
  print("BLE scanning started for door/window sensor relay control");
}
